...
async function uploadToIPFS(fileData: string, fileName: string, metadata?: Record<string, any>): Promise<{ cid: string; hash: string; url: string }> {
  try {
...
async function uploadJSONToIPFS(data: any, fileName: string): Promise<{ cid: string; hash: string; url: string }> {
  try {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/patient/records", async (req, res) => {
    try {
      ...
      // Add IPFS URL to each record based on fileCID
      const recordsWithUrls = records.map(record => ({
        ...record,
        ipfsUrl: record.fileCID && !record.fileCID.startsWith('metadata_') 
          ? ipfsService.getCIDUrl(record.fileCID)
          : null
      }));
  ...
  app.post("/api/patient/records", async (req, res) => {
    try {
      ...
      // Create record WITHOUT fileData - only store CID and metadata
      const record = await storage.createMedicalRecord({
        ...
        fileData: null, // Don't store fileData when using IPFS
      });
  ...
  app.get("/api/doctor/treatments", async (req, res) => {
    try {
      ...
      // Add IPFS URL to each treatment based on recordCID
      const treatmentsWithUrls = treatments.map(treatment => ({
        ...treatment,
        ipfsUrl: treatment.recordCID ? ipfsService.getCIDUrl(treatment.recordCID) : null
      }));
      
      res.json(treatmentsWithUrls);
    } catch (error) {
  ...
  app.post("/api/doctor/treatments", async (req, res) => {
    try {
      ...
      // Upload treatment files to IPFS individually and store only CIDs
      let treatmentFilesWithCID: Array<{name: string; type: string; cid: string; ipfsUrl: string}> | null = null;
      
      if (validatedData.treatmentFiles && validatedData.treatmentFiles.length > 0) {
        treatmentFilesWithCID = await Promise.all(
          validatedData.treatmentFiles.map(async (file, index) => {
            const ipfsResult = await uploadToIPFS(
              file.data,
              file.name,
              {
      ...
      // Upload treatment record to IPFS (without base64 file data, only CIDs)
      const treatmentData = {
        patientId: patient.id,
        doctorId: doctor.id,
        ...
        doctorSignature,
        signatureHash,
        treatmentFiles: treatmentFilesWithCID, // Store only CIDs, not base64 data
      };
      ...
      const treatmentLog = await storage.createTreatmentLog({
        ...
        doctorSignature,
        signatureHash,
        treatmentFiles: treatmentFilesWithCID, // Store CID-based files instead of base64
      });
  ...
  app.get("/api/hospital/treatments", async (req, res) => {
    try {
      ...
      // Add IPFS URL to each treatment based on recordCID
      const treatmentsWithUrls = hospitalTreatments.map(treatment => ({
        ...treatment,
        ipfsUrl: treatment.recordCID ? ipfsService.getCIDUrl(treatment.recordCID) : null
      }));
